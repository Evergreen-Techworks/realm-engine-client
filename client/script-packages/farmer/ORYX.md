...
- Castle walls are broken only where they block the way on. A destructible wall (by objects.xml type: Destructible Castle Wall, its Fortified and Ice variants) is chosen when it seals the reachable floor off from floor the player cannot reach, or from unseen tiles, and nothing reachable leads anywhere nearer the goal. The farmer walks up to it, then Auto Aim locks on it with its "Ignore walls" filters lifted for that lock only (`combat.aimAt(id, { includeStructures: true })`). The Auto Aim plugin's settings return when the lock ends. Walls sealing nothing are left alone. Walls, barrels and other square-occupying enemies are never fought as creatures. A wall that loses no HP for 15 seconds is reported in the script log.
- Guardian targets switch when one becomes invulnerable. No Janus detour.
...
Automated tests exercise sealing-wall selection and breaking on the real SDK (the Castle start room), progression, incorrect/locked portals, unlock consumption, loot deadlines, confirmed versus missing/revived bosses, guardian switching, all four miniboss branches, animation guards, authenticated phase cues, expiring ability pauses, static-wall corridor routing, room gates, dangerous terrain and map-state reset. They do not simulate the game's complete projectile field or certify encounter survival.
