...
## 2026-09-15 secondary-preference integration

Controller subsequently approved a bounded native seam to resolve the integration stop. The earlier helper-only commit is `02e3c5e`. This follow-up wires the helper into targetable MBC combat, without using an ordinary navigation waypoint for regrouping.

- `Dodge.setGroupPreference(bossId, x, y)` sends one atomic `scriptMbcGroupGoal` text command. It does not replace the combat target, alter lock-follow settings, or activate a walk goal. `clearGroupPreference` clears it. SDK, bridge, typed feature contract, and native registry agree.
- Native preference carries its boss ID and expires after 750 ms without refresh. Invalid, nonfinite, oversized, malformed, distant, wrong-lock, or reversed-clock input fails closed. Parser uses `strtoll` and `ERANGE`, including on Windows where `long` is only 32 bits. Map entry and disabling dodge clear preference state.
- Native optional group scoring is applied only after ordinary safe-candidate admission. Least-bad fallback scoring is untouched. A durable current stand permits a bounded group-directed step only when transit, terrain, enemy bodies, active zones, pending-zone cost, and a durable destination all pass. A soon-closing pocket does not justify optional regrouping.
- Boss aiming/annulus context remains present and `walkTo` stays false, retaining timed-escape eligibility. Manual and existing waypoint movement suppress this secondary preference. Worker snapshots carry the group owner/position; game-thread acceptance rejects mismatched results. Expiration or changed ownership forces a fresh live decision rather than retaining an old group-driven step.
- Lost Halls applies this only after locking a targetable Marble Colossus. Missing/stale group evidence clears it; vulnerable-phase loss, player death, encounter/map reset clear it. Floor reachability and cardinal/recentering steps remain those tested in the helper. No unknown-room template or unseen-shot prediction is added.

Test-first: two TypeScript integration failures became passing tests for separate boss aim/group commands and phase reset. Native reposition test failed before implementation. Review added a soon-closing-pocket regression (47 checks, one failure before the durable destination gate). Additional native checks cover ownership/TTL, reset, malformed input, enormous IDs, clock reversal, distant goal, closed projectile wall, and blocked terrain doorway.

Validation commands: SDK `npm run build`; client `npm test`, `npm run typecheck`, `npm run typecheck:tests`; `python3 internal/tests/run_udodge_zone_tests.py`; `git diff --check`. Client: 64 files / 598 tests. Bridge: 175 DLL keys / 156 client-sendable / 21 DLL-only / two known-unhandled. Native admission: 47 checks; remaining baseline counts and known dense-boss scenario limitations are unchanged.

This is a local tested slice, not a delivered portable or gameplay confirmation. The policy remains bounded: distant groups, invulnerable phases, missing identity/timestamps, and inaccessible floor do not create a regroup command. Global room routing and difficult multi-wall weaving remain open. No claim is made that the user's deaths are solved.
